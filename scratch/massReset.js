const admin = require('firebase-admin');
const pid = 'prodfastxcanwork';
const email = 'firebase-adminsdk-fbsvc@prodfastxcanwork.iam.gserviceaccount.com';
const pk = '-----BEGIN PRIVATE KEY-----\\YMclQpMJD3G1oNlg1aY1+jJSguBv1oCSu3DMEmC9dkwHGlG+jNyBESq1NX90Ip55\\n-----END PRIVATE KEY-----\n'.replace(/\\n/g, '\n');

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert({
            projectId: pid,
            clientEmail: email,
            privateKey: pk
        })
    });
}

const adminDb = admin.firestore();

async function resetAllOperators() {
    console.log("=== INIZIO RESET NUCLEARE DI TUTTI GLI OPERATORI ===");
    const opSnap = await adminDb.collection('operators').get();
    let batch = adminDb.batch();
    let count = 0;
    
    opSnap.forEach(doc => {
        batch.update(doc.ref, {
            activeJobId: null,
            activeJobDbId: null,
            activePhaseName: null,
            activePhaseId: null,
            stato: 'inattivo'
        });
        count++;
    });

    if (count > 0) {
        await batch.commit();
        console.log(`\n✅ RESET COMPLETATO! ${count} operatori svuotati e inattivi. Tutti i tablet sono ora formattati (schermata Home).`);
    } else {
        console.log(`\nNessun operatore trovato in database.`);
    }
}

resetAllOperators()
    .then(() => process.exit(0))
    .catch(err => {
        console.error(err);
        process.exit(1);
    });
