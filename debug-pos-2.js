const admin = require('firebase-admin');
const pid = 'prodfastxcanwork';
const email = 'firebase-adminsdk-fbsvc@prodfastxcanwork.iam.gserviceaccount.com';
const pk = '-----BEGIN PRIVATE KEY-----\\YMclQpMJD3G1oNlg1aY1+jJSguBv1oCSu3DMEmC9dkwHGlG+jNyBESq1NX90Ip55\\n-----END PRIVATE KEY-----\n'.replace(/\\n/g, '\n');

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert({
            projectId: pid,
            clientEmail: email,
            privateKey: pk
        })
    });
}

const db = admin.firestore();

async function checkPOsAndMat() {
    console.log('Checking material 50X005X33FR...');
    const matSnap = await db.collection('rawMaterials').where('code', '==', '50X005X33FR').get();
    if (matSnap.empty) {
        console.log('Material not found!');
        return;
    }
    const matId = matSnap.docs[0].id;
    console.log('Material ID:', matId);

    console.log('Checking POs...');
    const snapshot = await db.collection('purchaseOrders').where('materialCode', '==', '50X005X33FR').get();
    snapshot.docs.forEach(doc => {
        const data = doc.data();
        console.log({
            id: doc.id,
            materialId: data.materialId,
            materialCode: data.materialCode,
            isIdMatch: data.materialId === matId
        });
    });
}

checkPOsAndMat().then(() => process.exit(0)).catch(err => { console.error(err); process.exit(1); });
