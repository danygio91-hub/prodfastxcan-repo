const admin = require('firebase-admin');
const pid = 'prodfastxcanwork';
const email = 'firebase-adminsdk-fbsvc@prodfastxcanwork.iam.gserviceaccount.com';
const pk = '-----BEGIN PRIVATE KEY-----\\YMclQpMJD3G1oNlg1aY1+jJSguBv1oCSu3DMEmC9dkwHGlG+jNyBESq1NX90Ip55\\n-----END PRIVATE KEY-----\n'.replace(/\\n/g, '\n');

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert({
            projectId: pid,
            clientEmail: email,
            privateKey: pk
        })
    });
}

const db = admin.firestore();

async function checkTotalDemandFuzzy() {
    console.log('Checking fuzzy demand for 50X005X33FR...');
    const snapshot = await db.collection('jobOrders').get();
    let totalDemand = 0;
    const target = '50X005X33FR';
    snapshot.docs.forEach(doc => {
        const data = doc.data();
        const bom = data.billOfMaterials || [];
        bom.forEach(item => {
            const comp = (item.component || '').toUpperCase().trim();
            if (comp === target && item.status !== 'withdrawn') {
                totalDemand += (item.fabbisognoTotale || 0);
                if (item.component !== target) {
                    console.log(`Mismatch detected: "${item.component}" vs "${target}" in Job ${data.jobOrderNumber}`);
                }
            }
        });
    });
    console.log('Total Fuzzy Demand:', totalDemand);
}

checkTotalDemandFuzzy().then(() => process.exit(0)).catch(err => { console.error(err); process.exit(1); });
