...
export async function signPrivacyPolicy(operatorId: string): Promise<{ success: boolean; message: string }> {
  const operatorRef = adminDb.collection("operators").doc(operatorId);
  ...
  try {
    ...
    const policyVersion = policySnap.exists ? (policySnap.data() as any).lastUpdated.toMillis() : 'initial';
